...
def generate_transaction():
    ...
    return {
        ...
        "currency": random.choice(['USD','G8P']),
        "city": fake.city(),
        "country": fake.country(),
        "merchantName": fake.company(),
        "paymentMethod": random.choice(["credit_card","debit_card","online_transfer"]),
        "ipAddress": fake.ipv4(),
        "voucherCode": random.choice(['','DISCOUNT25','']),
    ...
    cur.execute(
        ...
        """, (transaction['transactionId'],transaction['userId'],datetime.fromtimestamp(transaction['timestamp']).strftime('%Y-%m-%d %H:%M:%S'),
            transaction['amount'],transaction['currency'],transaction['city'],transaction['country'],
              transaction['merchantName'],transaction['paymentMethod'],transaction['ipAddress'],transaction['voucherCode'],transaction['affiliateId'])
